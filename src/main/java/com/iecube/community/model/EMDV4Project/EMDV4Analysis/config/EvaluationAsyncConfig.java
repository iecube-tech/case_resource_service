...
@EnableAsync
@Configuration
public class EvaluationAsyncConfig implements AsyncConfigurer {
    /**
     * 自定义异步线程池
     */
    ...
    public Executor getAsyncExecutor() {
        ...
        executor.setQueueCapacity(200); // 队列容量  = 队列容量 = 核心线程数 × 单次任务平均耗时（秒） × 峰值QPS × 缓冲系数（0.5~1） 结果可向上取整
        executor.setKeepAliveSeconds(60); // 空闲线程存活时间
        executor.setThreadNamePrefix("agent-eva-"); // 线程名前缀
